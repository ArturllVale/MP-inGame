...
try {
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$payCreate) {
      throw new Exception("Não foi possível processar seu pagamento. Por favor, tente novamente mais tarde.");
    }
    ...
    $result = curl_exec($ch);
    if (curl_errno($ch)) {
      throw new Exception("Não foi possível validar os dados do seu cartão. Por favor, verifique as informações e tente novamente.");
    }
    curl_close($ch);
    ...
    $result = curl_exec($ch);
    if (curl_errno($ch)) {
      throw new Exception("Não foi possível conectar com o serviço de pagamento. Por favor, verifique sua conexão e tente novamente em alguns instantes.");
    }
    curl_close($ch);
    ...
    if (isset($response['status']) && $response['status'] == 'approved') {
      ...
      $mensagem_amigavel = "Seu pagamento não foi aprovado.";
      ...
      throw new Exception($mensagem_amigavel);
    }
  }
} catch (Exception $e) {
